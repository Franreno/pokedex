...
export async function findEvolutionChainArray(evolution){
    ...
    evolutionChainPokemons.push(pokemon);

    if(evolutionChain.evolves_to.length!==0){
        await evolutionChain.evolves_to.map(async evolutionChainIn=>{
            res = await fetch(evolutionChainIn.species.url);
            resPokemonSpecies = await res.json();
            ...
            if(evolutionChainIn.evolves_to.length!==0){
                await evolutionChainIn.evolves_to.map(async evolutionChainInIn=>{
                    res = await fetch(evolutionChainInIn.species.url);
                    resPokemonSpecies = await res.json();
